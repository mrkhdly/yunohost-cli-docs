...
| `show <path>` | Display log content | `-n NUMBER`, `--share` (deprecated, use `log share`), `-i` (filter irrelevant), `-s` (with sub-operations) |
| `display <path>` | Alias for `show` | Same options |
| `share <path>` | Share log on yunopaste (alias to `show --share`) | — |
